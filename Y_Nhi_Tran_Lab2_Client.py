...
def handle_state_selection():
    ...
    if state and start_year <= end_year:
        with Client(5000) as client:
            ...
            plt.xticks(list(years)[::5], rotation='vertical')
            plt.show()
    else:
...
start_year_label = tk.Label(root, text="In range 1970 - 2020\nStart Year:")
...
select_button = tk.Button(root, text="OK", command=handle_state_selection)
select_button.pack()
