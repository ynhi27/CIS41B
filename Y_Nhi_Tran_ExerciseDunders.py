...
class WebScraping:
    # __init__ - creation method - but not the converted method from Lab 0
    def __init__(self):
        self.soup = None
        self.rows = []
        self.dict = defaultdict(list)

    # __repr__ - represent an object
    def __repr__(self):
        return f'WebScraping(soup = {self.soup}, rows = {self.rows}, dict = {self.dict})'

    # __str__
    def __str__(self):
        return f'WebScraping object with soup = {self.soup}, rows = {self.rows}, dict = {self.dict}'
    ...
    # __add__ method - binary
    def __add__(self, other):
        new_dict = defaultdict(list)
        for d in [self.dict, other.dict]:
    ...
    def find_data(self):
        ...
        # Collect data
        for self.row in table.find_all('tr')[1:]:
            # Find all info of td - table data in the site
            column = self.row.find_all('td')
    ...
    # Convert print_data method to __str__
    def __str__(self):
        # Print collected data
        data_string = "Production-based emissions: annual carbon dioxide emissions" \
                      "in metric tons per capita in 1980 and 2018\n"
        data_string += "{:>46} {:>9} {:>10}".format("COUNTRY NAME", "1980", "2018")
        data_string += "\n"
        for k, v in self.dict.items():
            y1, y2 = v
            data_string += f'{k:>46}' + "{:>10} {:>10}".format(y1, y2)
            data_string += "\n"
        return data_string
    ...
    def print_data(self):
        # Find the data
        self.find_data()
        # Print the data that formatted in __str__
        print(self.__str__())


if __name__ == "__main__":
    openHtmlSite = WebScraping()
...
# Output
# Production-based emissions: annual carbon dioxide emissionsin metric tons per capita in 1980 and 2018
